...
from detector_eugene import ObjectDetector
...
class HLInterfaceController(Node):
    ...
    def _drive_to_goal(self, goal_xy):
        """Движение к цели через Nav2 вместо ручного управления скоростями."""
        ...
        goal_pose.pose.orientation.w = 1.0  # Нейтральный поворот

        # Если мы еще не в процессе навигации, даем команду
        if not self.is_navigating:
            self.get_logger().info(f"Nav2: едем к координатам {goal_xy}")
            self.nav.goToPose(goal_pose)
            self.is_navigating = True

        # Проверяем, приехал ли робот
        if self.nav.isTaskComplete():
            self.is_navigating = False
            result = self.nav.getResult()
            if result == TaskResult.SUCCEEDED:
                self.get_logger().info("Nav2: точка достигнута")
                return True
            else:
                self.get_logger().error("Nav2: ошибка навигации")
    ...
    def run_user_code(self):
        # 1. Проверка завершения всего задания
        ...
        # 2. Проверка наличия данных с камер
        ...
        # 4. Сценарий: Видим объект сейчас
        if target_det is not None:
            target_map = self._transform_point_to_map(target_det.xyz_camera)
            if target_map is not None:
                self.known_targets[target_id] = target_map
                
                # Публикуем в топик соревнования, если видим впервые
                if target_id not in self.reported_targets:
                    self.publish_detected_object(target_id)
                    self.reported_targets.add(target_id)

                # Если подкрались достаточно близко — переходим к следующему
                cam_pose = self._get_camera_pose_map()
                if cam_pose is not None:
                    dist = math.hypot(target_map[0] - cam_pose[0], target_map[1] - cam_pose[1])
                    if dist < 0.5: # 50 см до объекта
                        self.get_logger().info(f"Цель {target_id} взята!")
                        if self.is_navigating:
                            self.nav.cancelTask()
                            self.is_navigating = False
        ...
        # 5. Сценарий: Объекта не видно, но мы помним его координаты
        if target_id in self.known_targets:
            reached = self._drive_to_goal(self.known_targets[target_id][:2])
            if reached:
                self.current_target_idx += 1
            return

        # 6. Сценарий: Ничего не знаем об объекте — крутимся
        if self.is_navigating:
            self.nav.cancelTask()
            self.is_navigating = False
        self._explore()
