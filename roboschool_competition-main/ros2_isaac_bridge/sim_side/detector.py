...
DEFAULT_MODEL_PATH = _REPO_ROOT / "yolo26n.pt"
        ...
        if self._log_detections and _LOG.isEnabledFor(logging.INFO):
            ...
            if detections:
                for d in detections:
                    ...
                    _LOG.info(
                        "Найден класс %s (%s), id=%d, уверенность=%.3f, yolo=%s, orb=%s, "
                ...
                _LOG.info("Кадр: эфф. FPS=%.2f | %s", fps_eff, res)
            else:
                _LOG.info("Детекций нет | эфф. FPS=%.2f | %s", fps_eff, res)
