...
current_ros_cmd = {"vx": 0.0, "vy": 0.0, "wz": 0.0}

def ros_cmd_callback(msg):
    global current_ros_cmd
    current_ros_cmd["vx"] = msg.linear.x
    current_ros_cmd["vy"] = msg.linear.y
    current_ros_cmd["wz"] = msg.angular.z
...
def main():
    ...
    # Добавить перед началом цикла while
    last_detected_id = -1 

    # В callback моста или через прямую проверку сообщений
    def on_object_received(msg_data):
        nonlocal last_detected_id
        # Предполагаем, что в msg_data приходит целое число (ID объекта)
        last_detected_id = int(msg_data)

    # Подписываемся через объект bridge (зависит от реализации вашего SimBridgeClient)
    bridge.set_object_callback(on_object_received)
    ...
    while True:
        ...
        # --- ЗАМЕНА СТРОК 240-242 ---
        # Проверяем, пришел ли новый ID объекта через ROS-подписку
        # Значение обновляется асинхронно в callback-функции
        if last_detected_id != -1:
            log_detected_object(last_detected_id)
            # Сбрасываем, чтобы не логировать один и тот же момент срабатывания дважды
            last_detected_id = -1 
        # ----------------------------
